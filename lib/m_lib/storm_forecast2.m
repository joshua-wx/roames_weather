function [fcst_nl,fcst_graph_nl]=storm_forecast2(track_idx,storm_jstruct,kml_dir,region,start_td,stop_td,cur_vis,radar_id)
%WHAT
...
for i=1:length(end_cell_idx)
    
    %project track from end_cell_idx(i)
    keyboard
    [proj_azi,proj_arc,vil_dt,trck_stats,trck_dt] = project_storm_kml(end_cell_idx(i),storm_jstruct);
    
    %extract geometry of end cell
    dbz_centlat    = str2num(storm_jstruct(end_cell_idx(i)).storm_dbz_centlat.N)./geo_scale;
    dbz_centlon    = str2num(storm_jstruct(end_cell_idx(i)).storm_dbz_centlon.N)./geo_scale;
    end_orient     = str2num(storm_jstruct(end_cell_idx(i)).orient.N)./stats_scale;
    end_orient_x   = cosd(end_orient);
    end_orient_y   = -sind(end_orient);
    end_orient_n   = rad2deg(atan2(end_orient_x,end_orient_y));
    end_maj_axis   = str2num(storm_jstruct(end_cell_idx(i)).maj_axis.N)./stats_scale;
    end_maj_axis   = km2deg(end_maj_axis*h_grid/1000)/2;
    end_min_axis   = str2num(storm_jstruct(end_cell_idx(i)).min_axis.N)./stats_scale;
    ...
    for j=2:n_fcst_steps+1 %offset from start
        ...
        keyboard
        single_fcst_tag=['cell_fcst_',num2str((j-1)*fcst_step),'min_idx',num2str(ident2kml(end_cell_idx(i)).index)];
    ...
    tag     = ['cell_fcst_cell_ind',num2str(end_cell_idx(i)),'_IDR',num2str(radar_id)];
    ...
    tag = ['graph_cell_ind',num2str(end_cell_idx(i)),'_IDR',num2str(radar_id)];
