...
for i = 1:length(uniq_storm_rid_list)
    ...
    rid_time_list       = storm_mesh_list(rid_idx==i);
    ...
    for j = 1:length(uniq_rid_track_list)
        ...
        for k = 1:length(swath_mesh_threshold)
            ...
            check_mask = target_idx(track_mesh>=mesh_threshold);
        ...
        out_fn  = ['hail_',num2str(target_rid,'%02.0f'),'_',datestr(max(rid_time_list),'yyyymmdd_HHMMSS'),'.mat'];
        out_ffn = [impact_tmp_root,num2str(target_rid,'%02.0f'),'/',out_fn];
