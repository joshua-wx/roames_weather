...
for i=1:dataset_count
    %load ppi attributes
    [ppi_elv,ppi_time] = read_dataset_atts(h5_ffn,i,radar_id);
    elv_vec(i)         = ppi_elv;
...
function [ppi_elv,ppi_time]=read_dataset_atts(h5_ffn,dataset_no,radar_id)
...
try
    ...
    if radar_id ~= 99
        ...
        ppi_time_vec    = datevec(ppi_time);
        ppi_time_vec(6) = 0;
        ppi_time        = datenum(ppi_time_vec);
    end
catch
